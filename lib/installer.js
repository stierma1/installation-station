  ...
  async install(){
    ...
    for(var i in this.questions){
      ...
      if(silent){
       ...
       var defaultValue = this.questions[i].defaultValue;
       if(this.questions[i].format === "INT"){
         answer = parseInt(answer, 10);
         if(isNan(answer) && defaultValue !== undefined){
           answer = defaultValue;
         }
       }
       else if(this.questions[i].format === "HEX_INT"){
         answer = parseInt(answer, 16);
         if(isNan(answer) && defaultValue !== undefined){
           answer = defaultValue;
         }
       }
       else if(this.questions[i].format === "FLOAT"){
         answer = parseFloat(answer);
         if(isNan(answer) && defaultValue !== undefined){
           answer = defaultValue;
         }
       }
       else if(this.questions[i].format === "DATETIME"){
         if(answer === ""){
           answer = new Date(defaultValue).toISOString();
         }
